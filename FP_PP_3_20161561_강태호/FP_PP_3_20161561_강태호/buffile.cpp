...
int BufferFile::Update(int recaddr, int diff)
{
	...
	if (diff == 0) {
		...
		unsigned short s_dff = (unsigned short)(diff - 2);
		Buffer.DWrite(File, recaddr);
		File.write((char*)&s_dff, sizeof(s_dff));
		File.write("*", 1);
		return recaddr + 3;
	}
	else {
...
// write the current buffer at the end of File
{
	...
	addr = Buffer.FindSpace(File, &prev, &next, &diff);
	if (addr == -1) {
		return Buffer.DWrite(File, -1);
	}
	else {
		Buffer.WriteDeleteAddr(File, prev, next);
		Buffer.DWrite(File, addr);
		if (diff != 0) {

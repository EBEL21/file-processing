...
{public:
	...
	int Open_by_dat (char * name, int mode=ios::in|ios::out);
	int Close ();
...
template<class RecType>
inline int TextIndexedFile<RecType>::Delete(char * key)
{
	int ref = Index.Search(key);
	if (ref == -1) return -1;
	DataFile.Delete(ref);
	int result = Index.Remove(key);
...
{	
	...
	if (ref < 0) return -1;
	int newRef = DataFile.Update(record, ref);
	Index.Modify(oldKey, newRef);
...
template<class RecType>
int TextIndexedFile<RecType>::Open_by_dat(char * fileName, int mode)
{
	int result;
	char * dataFileName, *indexFileName;
	result = SetFileName(fileName, dataFileName, indexFileName);
